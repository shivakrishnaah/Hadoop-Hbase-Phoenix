...
# java
RUN curl -LO 'http://download.oracle.com/otn-pub/java/jdk/8u102-b14/jdk-8u102-linux-x64.rpm' -H 'Cookie: oraclelicense=accepmt-securebackup-cookie'
RUN rpm -i jdk-8u102-linux-x64.rpm
RUN rm jdk-8u102-linux-x64.rpm

ENV JAVA_HOME /usr/java/default
ENV PATH $PATH:$JAVA_HOME/bin
ENV echo "export JAVA_HOME=$JAVA_HOME" >> /etc/profile
ENV echo "export PATH=$PATH:$JAVA_HOME/bin" >> /etc/profile
# hadoop
...
# hbase
RUN curl -s https://www.apache.org/dist/hbase/1.2.3/hbase-1.2.3-bin.tar.gz | tar -xz -C /usr/local/
RUN cd /usr/local && ln -s ./hbase-1.2.3 hbase
...
RUN curl https://www.apache.org/dist/zookeeper/zookeeper-3.4.6/zookeeper-3.4.6.tar.gz | tar -xz -C /usr/local/
...
RUN curl http://apache.mirror.anlx.net/phoenix/apache-phoenix-4.8.0-HBase-1.2/bin/apache-phoenix-4.8.0-HBase-1.2-bin.tar.gz | tar -xz -C /usr/local/
RUN cd /usr/local && ln -s ./apache-phoenix-4.8.0-HBase-1.2-bin phoenix
RUN cp /usr/local/apache-phoenix-4.8.0-HBase-1.2-bin/phoenix-4.8.0-HBase-1.2-server.jar $HBASE_HOME/lib/
